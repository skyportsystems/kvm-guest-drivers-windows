...
for %%A in (pvpanic vioserial) do (
  for %%B in (Win10 Win8 Win7) do (
    call %SIGNTOOL% sign /v /s %%1 /t http://timestamp.verisign.com/scripts/timestamp.dll %%A/Install/%%B/amd64/*.sys 
    if %ERRORLEVEL% NEQ 0 goto signfail 
    call %SIGNTOOL% sign /v /s %%1 /t http://timestamp.verisign.com/scripts/timestamp.dll %%A/Install/%%B/amd64/*.cat 
    if %ERRORLEVEL% NEQ 0 goto signfail 
  )
)
